...
def get_objective_value(data, params, err, method='no_fse'):
    """获取datacollapse库使用的目标函数值（加权残差平方和）"""
    ...
    try:
        ...
        # 使用加权样条拟合
        spline = UnivariateSpline(x_sorted, y_sorted, w=w_sorted, s=len(x)*1e-6)
        y_fit = spline(x_sorted)
        
        # 计算加权残差平方和（这就是datacollapse库的目标函数）
        resid = y_sorted - y_fit
        objective_value = float(np.sum(w_sorted * resid**2)) / len(Y)
        
        return objective_value
        
    except Exception as e:
        print(f"    Warning: Could not compute objective value: {e}")
        return np.nan
...
def generate_all_plots():
    ...
    try:
        ...
        # 获取目标函数值
        objective_no_fse_all = get_objective_value(data_all, params_no_fse_all, err_all, 'no_fse')
    ...
    try:
        ...
        # 获取目标函数值
        objective_no_fse_no_7 = get_objective_value(data_no_7, params_no_fse_no_7, err_no_7, 'no_fse')
    ...
    try:
        ...
        # 获取目标函数值
        objective_no_fse_no_7_9 = get_objective_value(data_no_7_9, params_no_fse_no_7_9, err_no_7_9, 'no_fse')
    ...
    try:
        ...
        # 获取目标函数值
        objective_fse = get_objective_value(data_all, params_fse, err_all, 'fse')
...
def main():
    ...
    print(f"\nNote: Objective value is the weighted residual sum of squares used by datacollapse library")
    print(f"This is the actual quality measure for data collapse, not χ²")
